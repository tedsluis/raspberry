...
# Greetings
greetings="$borderBar$(color $greetingsColor "$(center "Welkom terug, $me!")")$borderBar\n"
...
read loginFrom loginIP loginDate <<< $(last -F $me | awk 'NR==2 { print $2," (" $3 ") ",$4 " " $5 " " $6 " " $8 " / " $7 }')


if [[ $loginFrom == tty ]]; then
  read loginFrom loginIP loginDate <<< $(last -F $me | awk 'NR==2 { print $2,"local",$3 " " $4 " " $5 " " $7 " / " $6 }')
fi
